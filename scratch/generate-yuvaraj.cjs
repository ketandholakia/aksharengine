...
const a1Str = fs.readFileSync(path.join(__dirname, 'yuvaraj-a1.txt'), 'utf16le');
const a2Str = fs.readFileSync(path.join(__dirname, 'yuvaraj-a2.txt'), 'utf16le');
...
const profile = {
  ...
  name: "Yuvaraj",
  script: "devanagari",
  language: "mr", // Marathi heavily uses Shivaji/Yuvaraj mappings
  version: "1.0.0",
  fontFamilies: ["Yuvaraj"],
  isBuiltIn: true,
  reorderingRules: {
    leftMatraSymbols: ["i"],
    rephSymbols: ["-"],
...
fs.writeFileSync(
  path.join(__dirname, '../public/profiles/yuvaraj.json'),
  JSON.stringify(profile, null, 2)
);

console.log("Successfully built public/profiles/yuvaraj.json");
