...
const a1Str = fs.readFileSync(path.join(__dirname, 'suchi-a1.txt'), 'utf16le');
const a2Str = fs.readFileSync(path.join(__dirname, 'suchi-a2.txt'), 'utf16le');
...
const profile = {
  ...
  language: "hi",
  version: "1.0.0",
  fontFamilies: ["Suchi-Dev"],
  isBuiltIn: true,
  reorderingRules: {
    leftMatraSymbols: ["×", "Û", "Ø"], 
    rephSymbols: ["Ô"], 
    customTransforms: []
  },
  mappings: mappings
...
fs.writeFileSync(
  path.join(__dirname, '../public/profiles/suchi-dev.json'),
  JSON.stringify(profile, null, 2)
);

console.log("Successfully built public/profiles/suchi-dev.json");
