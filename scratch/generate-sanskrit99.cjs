...
const a1Str = fs.readFileSync(path.join(__dirname, 'sanskrit99-a1.txt'), 'utf16le');
const a2Str = fs.readFileSync(path.join(__dirname, 'sanskrit99-a2.txt'), 'utf16le');
...
const profile = {
  ...
  name: "Sanskrit 99",
  script: "devanagari",
  language: "sa", // Sanskrit/Hindi
  version: "1.0.0",
  fontFamilies: ["Sanskrit 99"],
  isBuiltIn: true,
  reorderingRules: {
    leftMatraSymbols: ["i"],
    rephSymbols: ["R"],
...
fs.writeFileSync(
  path.join(__dirname, '../public/profiles/sanskrit99.json'),
  JSON.stringify(profile, null, 2)
);

console.log("Successfully built public/profiles/sanskrit99.json");
