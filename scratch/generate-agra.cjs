...
const a1Str = fs.readFileSync(path.join(__dirname, 'agra-a1.txt'), 'utf16le');
const a2Str = fs.readFileSync(path.join(__dirname, 'agra-a2.txt'), 'utf16le');
...
fs.writeFileSync(
  path.join(__dirname, '../public/profiles/agra.json'),
  JSON.stringify(profile, null, 2)
);

console.log("Successfully built public/profiles/agra.json");
