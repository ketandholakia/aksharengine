...
const a1Str = fs.readFileSync(path.join(__dirname, 'yogesh-a1.txt'), 'utf16le');
const a2Str = fs.readFileSync(path.join(__dirname, 'yogesh-a2.txt'), 'utf16le');
...
const profile = {
  id: "dv-ttyogesh",
  name: "DV-TTYogesh (Hindi)",
  script: "devanagari",
  language: "hi",
  version: "1.0.0",
  fontFamilies: ["DV-TTYogesh", "DV-TTYogeshEN", "DVB-YogeshEN"],
  isBuiltIn: true,
  reorderingRules: {
    leftMatraSymbols: ["Ê", "Î", "Ë"], 
...
fs.writeFileSync(
  path.join(__dirname, '../public/profiles/dv-ttyogesh.json'),
  JSON.stringify(profile, null, 2)
);

console.log("Successfully built public/profiles/dv-ttyogesh.json");
